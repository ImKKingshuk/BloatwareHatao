...
impl PackageManager {
    ...
    pub async fn is_installed(&self, package: &str) -> Result<bool> {
        let output = self
            .adb
            .shell(&format!("pm list packages {}", package))
            .await?;
    ...
    pub async fn get_all_app_labels(
        &self,
        packages: &[String],
        ...
        for package in packages {
            let label = self
                .get_app_label(package)
                .await
                .unwrap_or_else(|_| extract_app_name(package));
    ...
    pub async fn get_package_info(&self, package: &str) -> Result<Package> {
        ...
        let dumpsys = self
            .adb
            .shell(&format!(
                "dumpsys package {} | grep -E 'versionName|versionCode'",
                package
            ))
            .await
            .unwrap_or_default();
        ...
        for line in dumpsys.lines() {
            ...
            if let Some(value) = line.strip_prefix("versionName=") {
                ...
                version_code = value.split_whitespace().next().and_then(|s| s.parse().ok());
            }
        }
    ...
    pub async fn disable(&self, package: &str) -> Result<OperationResult> {
        ...
        let output = self
            .adb
            .shell(&format!("pm disable-user --user 0 {}", package))
            .await?;
