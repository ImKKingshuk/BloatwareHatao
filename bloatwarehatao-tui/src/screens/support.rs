...
impl SupportScreen {
    ...
    fn draw_content(f: &mut Frame, area: Rect, app: &App) {
        // Create list items
        let items: Vec<ListItem> = app.state.support.items
            .iter()
            .enumerate()
            .map(|(i, item)| {
                ...
                let label_span = Span::styled(
                    ...
                    if is_selected { Style::default().fg(Color::Black).add_modifier(Modifier::BOLD) } 
                    else { Style::default().fg(label_color).add_modifier(Modifier::BOLD) }
                );
        ...
        let list = List::new(items)
            .block(list_block)
            .highlight_style(Style::default().add_modifier(Modifier::BOLD).bg(Color::DarkGray)) // Backup highlight
            .highlight_symbol("▶ ");
